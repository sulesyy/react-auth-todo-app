...
function App() {
  ...
  const handleAddTask = () => {
    ...
    const newTaskObj = { id: Date.now().toString(), title: newTask, description: "" };
    setTasks((prev) => ({ ...prev, draft: [...prev.draft, newTaskObj] }));
    setNewTask("");
  };
  ...
  return (
    <div className="app-container">
      <div className="top-bar">
        <h1 className="app-title">ToDoList <span className="user-email">({activeUser})</span></h1>
            ...
            <Droppable key={key} droppableId={key}>
              {(provided) => (
                <div className="column" ref={provided.innerRef} {...provided.droppableProps}>
                  <h2>{key.replace(/^\w/, (c) => c.toUpperCase())}</h2>
